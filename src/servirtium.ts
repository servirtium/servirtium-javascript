...
export class Servirtium {
  ...
  private _onProxyReq = async (
    ...
    Object.keys(this.callerRequestHeaderReplacements).forEach((regexString) => {
      ...
      Object.keys(headers).forEach((headerKey) => {
        ...
        const line2 = line.replace(
          regex,
          this.callerRequestHeaderReplacements[regexString]
        );
        if (line != line2) {
    ...
    Object.keys(this.recordRequestHeaderReplacements).forEach((regexString) => {
      ...
      Object.keys(callerRequestHeaders).forEach((headerKey) => {
        ...
        const line2 = line.replace(
          regex,
          this.recordRequestHeaderReplacements[regexString]
        );
        if (line != line2) {
    ...
    if (request.body) {
      const callerBody = this._replaceContent(
        request.body,
        this.callerRequestBodyReplacement
      );
      // Assign record body
  ...
  private _onProxyRes = async (
    ...
    Object.keys(this.recordResponseHeaderReplacements).forEach(
      (regexString) => {
        const regex = new RegExp(regexString, "g");
        Object.keys(proxyRes.headers).forEach((headerKey) => {
          ...
          const line2 = line.replace(
            regex,
            this.recordResponseHeaderReplacements[regexString]
          );
          if (line != line2) {
    ...
    proxyRes.on("end", async () => {
      ...
      const finalContent = this._replaceContent(
        content,
        this.recordResponseBodyReplacement
      );
      ...
      Object.keys(this.callerResponseHeaderReplacements).forEach(
        (regexString) => {
          const regex = new RegExp(regexString, "g");
          Object.keys(proxyRes.headers).forEach((headerKey) => {
            const line = headerKey + ": " + proxyRes.headers[headerKey];
      ...
      const callerResponseBody = this._replaceContent(
        finalContent,
        this.callerResponseBodyReplacement
      );
  ...
  private _playbackHandler = async (
    ...
    try {
      ...
      const { body, headers, statusCode } =
        this._getPlaybackResponse(interaction);
  ...
  public checkMarkdownIsDifferentToPreviousRecording =
